...
//--- 後台對話傳送
if ($_GET["serback_admin_msg"]){
	$check = $conn->Execute("INSERT INTO `".PREFIX."cache` (`id`, `type`, `from_id`, `to_id`, `data`, `create_date`) VALUES (NULL, 'serback_chat', '".$_SESSION["admin_info"]["id"]."', '".$_GET["serback_admin_msg"]."', '".$_POST["data"]."', '".date("Y-m-d H:i:s")."');");
}

//-- 對話資料接收
if ($_GET["serback_admin_msg_list"]){
	$conn->Execute("UPDATE ".PREFIX."cache SET create_date='".date("Y-m-d H:i:s")."' where type='serback_login' and from_id='".$_SESSION["admin_info"]["id"]."'");
	$data = $conn->GetArray("select *,IF(from_id='".$_SESSION["admin_info"]["id"]."',to_id,from_id) as sid from ".PREFIX."cache where type='serback_chat' and (from_id='".$_SESSION["admin_info"]["id"]."' or to_id='".$_SESSION["admin_info"]["id"]."') order by create_date");
	if ($data)
		foreach ($data as $k=>$v)
			$dataoutput[$v["sid"]][] =$v;
	echo json_encode($dataoutput);exit;
}

//-- 後台對話已讀取
if ($_GET["serback_admin_msg_isview"]){
	$conn->Execute("UPDATE ".PREFIX."cache SET isview=1 where id='".$_GET["serback_admin_msg_isview"]."' and to_id='".$_SESSION["admin_info"]["id"]."'");
}
