...
//--運費設定
$post_fee = $console->conn->GetRow("select * from ".PREFIX."setting WHERE type='post_fee' and lang = '".quotes($lang)."'");//--運費
$no_post_fee = $console->conn->GetRow("select * from ".PREFIX."setting WHERE type='no_post_fee' and lang = '".quotes($lang)."'");//--免運費金額
$post_fee2 = $console->conn->GetRow("select * from ".PREFIX."setting WHERE type='post_fee2' and lang = '".quotes($lang)."'");//--到達金額運費
...
$active = $console->conn->GetArray("select * from ".PREFIX."data_list where type='shop_active_any' and status=1 order by sort");
if ($active) {
...
//--現折設定
$now_desh = $console->conn->GetArray("select * from ".PREFIX."data_list where type='active_desh' and status=1 and active_on_date<='".date('Y-m-d')."' and active_off_date>='".date("Y-m-d")."'");
if ($now_desh)
...
//--通知信件資料
$mail_data = $console->conn->GetArray("select * from ".PREFIX."data_list where type='paymail' and status=1");
if ($mail_data)
...
//---針對會員型別進行折扣輸入設定
if ($member->getinfo()){
	$temp_memeber_type = $console->conn->GetRow("select * from ".PREFIX."data_list where type='memberdesh' and new_type='".$member->getinfo("type")."'");
...
if ($_GET["addpro"]){
	...
	//---會員分類折扣
	$member_dp = $console->conn->GetRow("select * from ".PREFIX."data_list where type='memberdesh' and new_type='".$member->getinfo('type')."' and status=1");
	$shopping_car->deshprice($member_dp['detail'],false);
	
	foreach ($temp_addpro as $k=>$v){
		
		//--取得分類折扣
		$pro_data = $console->conn->GetRow("select * from ".PREFIX."products where id='".$v."'");
		$cat_desh = $console->conn->GetRow("select * from ".PREFIX."category where id in(".str_replace('|__|',',',$pro_data["class"]).") order by desh desc");
...
if ($_GET["list"]){
	...
	if ($car){
		...
		$temp = $console->conn->GetRow("select * from ".PREFIX."products where id in(".implode(',',$pro_aray).") and close_ezship=1");
	...
	if ($member->getinfo('type')!='vip' || strtotime($member->getinfo('mode_time'))<=strtotime(date('Y-m-d H:i:s'))){ //--判斷不是VIP或者時間已到
		...
		//--
		$vip_m = $console->conn->GetRow("select * from ".PREFIX."data_list where type='memberdesh' and new_type='vip' and status=1");
		if ($vip_m){
...
//--產品加購選單
$data["addpro"] = $console->conn->GetArray("select * from ".PREFIX."category where root_id='2' and depth=1 and status=1 order by sort");
if ($data["addpro"])
	foreach ($data["addpro"] as $k=>$v){
		$data["addpro"][$k]["product"] = $console->conn->GetArray("select * from ".PREFIX."products where class like '%".$v["id"]."%' and type='products2' and status=1 order by sort");
