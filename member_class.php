...
欄位更新
$obj->row_data_chagnge($data,row,act) value , row_nam , +/*-
...
class member
{
		...
		function __construct($conn,$table)
		{
			...
			if ($_SESSION["login_info"]){
				$this->session = $this->conn->GetRow("select * from ".$this->table." WHERE id='".$_SESSION["login_info"]["id"]."'");
			}
			//--取得資料表內所有欄位 比對傳入的資料
		...
		function getmember($where=NULL) //--獲取依照條件尋找使用者
		{
		...
		function update($data){//--修改會員資料
				...
				$_SESSION["login_info"] = $this->session = $this->conn->GetRow("select * from ".$this->table." WHERE id=".$this->session["id"]);
				return $am;
		}
		...
		function newjoin($data,$point=0,$net_link=false){
			...
			if ($this->check_mail!=0){
				...
				$_SESSION["login_info"] = $this->session = $this->conn->GetRow("select * from ".$this->table." WHERE account='".$data["account"]."'");
				return $this->session;
			}
		}
		//--自動建立欄位資料
		...
		function recive_check_mail($data){
			...
			if ($temp_data){
				...
				$this->session = $_SESSION["login_info"] = $this->conn->GetRow("select * from ".$this->table." where id='".$temp[2]."' and status='1'");
				if ($this->session){
					return $this->session;
				}else{
					$this->erromsg = '異常無法登入!!';
...
class order_center
{
		...
		function __construct($conn,$table,$table2,$table3,$table4='')
		{
			...
			//--非會員時自動帶入暫存購買
			if ($_SESSION["login_info"]["id"]==NULL || $_SESSION["login_info"]["id"]=='0'){
				$_SESSION["login_info"]["id"] = '0';
				
				//-判斷是否存在暫存購物車
				if ($_SESSION["temp_order_no"]){
					$temp = $conn->GetRow("select * from ".$table." where status=1 and step='1' and order_no='".$_SESSION["temp_order_no"]."'");
				}
				
				if (!$_SESSION['temp_order_no'] || !$temp){
					$cardata["member_id"] = $_SESSION["login_info"]["id"];
					$_SESSION["temp_order_no"] = $cardata["order_no"] = strtoupper(base_convert(date('ymdhis'),10,16));//dechex()
					$cardata["status"] = 1;
					$cardata["create_date"] = date("Y-m-d H:i:s");
					$conn->AutoExecute($this->table,$cardata,"INSERT");
				}

			}else{
				unset($_SESSION["temp_order_no"]);
				//--檢查是否有購物車 沒有就創建一個
				$temp = $conn->GetRow("select * from ".$table." where status=1 and step='1' and member_id='".$_SESSION["login_info"]["id"]."'");	
				if (!$temp && $_SESSION["login_info"]["id"]!=NULL){
					$cardata["member_id"] = $_SESSION["login_info"]["id"];
		...
		function order_auto_set($title,$count=10,$mask='1'){
			...
			if ($_SESSION["temp_order_no"])
			$_SESSION["temp_order_no"] = $ORDER_NO;
		...
		function reload(){
			...
			if($_SESSION["companyto"]){
				$car["company"] = $_SESSION["companyto"];
			}
		...
		*/
		...
		function order_list($value=NULL,$star=0,$mount=NULL){
			...
			if ($value!=NULL && is_numeric($value)){
				...
				$temp = $this->conn->GetArray("select * from ".$this->table." where status!=0 and step='2' and ".$value." and member_id='".$_SESSION["login_info"]["id"]."' order by create_date desc".$limitsql);
			}else{
				$temp = $this->conn->GetArray("select * from ".$this->table." where status!=0 and step='2' and member_id='".$_SESSION["login_info"]["id"]."' order by create_date desc".$limitsql);
			}
			return $temp;
		}
...
//-金流延展庫
include_once(APP_PATH."paymode_class.php");
?>