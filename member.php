...
//--社群按鈕
$data["twitterurl"] = '//'.$_SERVER['HTTP_HOST'].'/includes/twitter/twitterapi.php?callback='.urlencode('//'.$_SERVER['HTTP_HOST'].'/member.php?act=net_login');
$data["yahoourl"] = '//'.$_SERVER['HTTP_HOST'].'/includes/openid/yahooapi.php?callback='.urlencode('//'.$_SERVER['HTTP_HOST'].'/member.php?act=net_login');
$data["fburl"] = '//'.$_SERVER['HTTP_HOST'].'/includes/openid/fbapi.php?callback='.urlencode('//'.$_SERVER['HTTP_HOST'].'/member.php?act=net_login');
$data["googleurl"] = '//'.$_SERVER['HTTP_HOST'].'/includes/googleapi/googleapi.php?callback='.urlencode('//'.$_SERVER['HTTP_HOST'].'/member.php?act=net_login');
...
switch ($act){
	...
	case "detail":
		...
		//--級別
		$temp = $conn->GetRow("select * from ".PREFIX."data_list where type='memberdesh' and new_type='".$_SESSION["member_info"]["member"]["type"]."'");
		$_SESSION["member_info"]["member"]["type_html"] = $temp["name"];
		
		if ($_POST){
			unset($_POST["account"]);
			if ($member->update($_POST)){
				alert('修改成功!!','?act=detail');
				exit;
			}
		}
	break;
	...
	case "fotgot":
		if ($_GET["auth"]){
			if ($member->recive_check_mail($_GET)){
				...
				echo '<form id="form1" action="member.php?act=cpw&nock=1" method="post"><input type="hidden" name="password" value="'.base64_decode(urldecode($_GET["auth"])).'"><br>';
				...
				alert('資訊來源錯誤!!請重新發送密碼驗證信件!!','index.php');
			}
			exit;
		}
		...
		if ($new_pd) alert('已寄出密碼修改信件!!',"?act=detail");
	break;
	...
	case "order":
		...
		foreach ($data["orderlist"] as $k=>$v){
				...
				switch ($v["pay_status"]){
					...
					break;
					default:
						$data["orderlist"][$k]["status_html"] = '<a href="shopping.php?payorder='.$v["id"].'">待付款';
					break;
				}
	...
	case "open":
		if ($member->recive_check_mail($_GET)){
			alert("會員帳戶認證完成!!","index.php?class=product");
		}else{
			alert($member->erromsg,'index.php');
			exit;
		}
	break;
